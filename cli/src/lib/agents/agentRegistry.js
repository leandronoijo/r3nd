 ...
 */
function getSummaryLogInstructions(agentName, doneFile) {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').replace('T', '-').substring(0, 19);
  const summaryFile = `rnd/agent_summaries/${agentName}-${timestamp}.md`;
