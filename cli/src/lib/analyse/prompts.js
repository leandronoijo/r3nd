...
function buildTargetedAppPrompt(appDir = '.') {
  return `You are an assistant that inspects a specific app/service directory and produces detailed instructions for that app.

Target directory: ${appDir}

Produce a markdown document with a top metadata block (YAML or JSON fenced block) that contains at minimum these camelCase fields:
- name: short canonical name for the app (inferred from the directory structure or package.json/etc.)
- applyTo: the path or glob the instructions apply to (use the target directory path)
...
module.exports = { buildOverviewPrompt, buildAppPrompt, buildTargetedAppPrompt };
