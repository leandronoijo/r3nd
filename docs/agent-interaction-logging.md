...
4. Create a summary log in `rnd/agent_summaries/` for non-retro commands
...
**Note:** The `<spec-dir>` is determined by the `spec-dir-name` setting in `r3nd.yaml`. The default is `r3nd` for newly scaffolded projects, while the seed repository uses `rnd`. The Cursor commands in this repository use `rnd/` paths.
...
4. Create summary log file for non-retro commands
