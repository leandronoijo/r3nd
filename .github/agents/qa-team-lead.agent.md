...
2. **Service orchestration:** When executing or describing how to run E2E tests, use Docker Compose to bring up required services. Tests and helper agents must not instruct developers or CI to start services manually (for example: `npm run dev` or starting individual servers by hand). Prefer `docker compose up --build` (or `docker-compose up --build`) and document any specific service names or compose files required.
