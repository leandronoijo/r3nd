...
Drive a feature from technical design to a verified implementation by coordinating build-plan generation when needed, dependency-aware task execution, task-level QA gates, and final E2E QA.
...
8. Mark the run successful only when final E2E QA status is `PASS`.
